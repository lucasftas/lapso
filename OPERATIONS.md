...
- [x] **Marketplace PUBLICADO**: o PAT do `vsce` segue inválido (`TF400813`), mas o upload saiu pelo **portal manage no navegador logado** (sessão do Brave, SSO passou direto). `lapso-0.4.4.vsix` aceito → `Verifying 0.4.4` → **galeria servindo 0.4.4 em 3min30** (`lastUpdated 2026-09-12T20:03:50Z`, pacote disponível pra download), conferido pela API `extensionquery` — não pela tela. Fecha o risco de o Settings Sync reverter a instalação local pra 0.3.1. ⚠️ Achado do caminho: o reCAPTCHA invisível do portal **loga erro** no navegador automatizado (`requestStorageAccess: Permission denied` + CSP barrando `recaptcha/api2/clr`) — parece falha, mas **não bloqueia** o envio; o que engana é a listagem continuar mostrando a versão antiga até a verificação terminar.
